...
} from 'src/app/pages/akuntansi/salk/saldo-awal-arus-kas/saldo-awal-aruskas-form/saldo-awal-aruskas-form.component';
import {SaldoawallakService} from 'src/app/core/services/saldoawallak.service';

@Component({
  selector: 'app-saldo-awal-arus-kas',
  templateUrl: './saldo-awal-arus-kas.component.html',
  styleUrls: ['./saldo-awal-arus-kas.component.scss'],
  providers: [InputRupiahPipe]
})
export class SaldoAwalArusKasComponent implements OnInit, OnDestroy {
  title: string = '';
  ...
  @ViewChild(SaldoAwalAruskasFormComponent, { static: true }) Form: SaldoAwalAruskasFormComponent;
  ...
  delete(e: any) {
    let message = "";
    if(e.idrekNavigation){
      message = `${e.idrekNavigation.kdper.trim()} - ${e.idrekNavigation.nmper.trim()}`;
    }
    this.notif.confir({
      message: `${message} Akan Dihapus ?`,
      accept: () => {
