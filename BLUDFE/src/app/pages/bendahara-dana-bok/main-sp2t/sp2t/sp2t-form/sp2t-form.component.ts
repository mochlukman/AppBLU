...
@Component({
  selector: 'app-sp2t-form',
  templateUrl: './sp2t-form.component.html',
  styleUrls: ['./sp2t-form.component.scss'],
  providers: [ InputRupiahPipe ]
})
export class Sp2tFormComponent implements OnInit {
  showThis: boolean;
  ...
  getStattrs(){
    ...
    this.stattrsService.getBylist('77,78')
      .subscribe(resp => {
  ...
  lookBendahara(){
    ...
    this.Bendahara.gets(this.forms.value.idunit, '03');
  ...
  simpan(){
    if(this.forms.valid){
      ...
      this.forms.patchValue({
        tglskp: this.forms.value.tglskp !== null ? new Date(this.forms.value.tglskp).toLocaleDateString('en-US') : null,
        tgltempo: this.forms.value.tgltempo !== null ? new Date(this.forms.value.tgltempo).toLocaleDateString('en-US') : null,
