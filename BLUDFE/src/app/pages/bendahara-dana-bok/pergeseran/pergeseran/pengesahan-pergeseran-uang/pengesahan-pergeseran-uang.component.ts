...
} from 'src/app/pages/bendahara-dana-bok/pergeseran/pergeseran/pengesahan-pergeseran-uang/pengesahan-pergeseran-uang-form/pengesahan-pergeseran-uang-form.component';
...
export class PengesahanPergeseranUangComponent implements OnInit, OnChanges, OnDestroy {
  ...
  get(){
    if(this.formFilter.valid && this.tabIndex == 1){
      ...
      this.loading = true;
      this.listdata = [];
      this.service.gets(this.formFilter.value.idunit, this.formFilter.value.idbend, '85,86')
        .subscribe(resp => {
          this.listdata = [];
          if(resp.length > 0){
            this.listdata = [...resp];
