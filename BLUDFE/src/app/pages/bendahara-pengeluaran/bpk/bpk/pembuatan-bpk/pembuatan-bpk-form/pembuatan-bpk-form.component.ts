...
export class PembuatanBpkFormComponent implements OnInit {
  ...
  getStattrs(){
    ...
    this.stattrsService.getBylist('21')
      .subscribe(resp => {
  ...
  changeStattrs(e: any){
    if(e.value){
      ...
      this.service.newNomorUrut(this.forms.value).subscribe(resp => {
        if(resp.status == 200){
          this.forms.patchValue({
            nobpk: `${resp.body['nomor']}/BPK-${label_split[0].trim()}/${this.unitSelected.kdunit}/${this.userInfo.NmTahun}`,
          });
        } else {
          this.forms.patchValue({
  ...
  simpan(){
    if(this.forms.valid){
      this.loading_post = true; 
      this.forms.patchValue({
        tglbpk: (this.forms.value.tglbpk !== null || this.forms.value.tglbpk !== '') ? new Date(this.forms.value.tglbpk).toLocaleDateString('en-US') : null
      });
      if(this.mode === 'add'){
