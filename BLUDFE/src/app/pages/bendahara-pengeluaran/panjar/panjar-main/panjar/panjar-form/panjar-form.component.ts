...
export class PanjarFormComponent implements OnInit {
  ...
  changeStattrs(e: any){
    if(e.value){
      ...
      this.service.newNomorUrut(this.forms.value).subscribe(resp => {
        if(resp.status == 200){
          this.forms.patchValue({
            nopanjar: `${resp.body['nomor']}/PANJAR/${this.unitSelected.kdunit}/${this.forms.value.jabbend}/${this.userInfo.NmTahun}`,
          });
        } else {
          this.forms.patchValue({
            nopanjar: `XXXXX/PANJAR/${this.unitSelected.kdunit}/${this.forms.value.jabbend}/${this.userInfo.NmTahun}`,
  ...
  simpan(){
    if(this.forms.valid){
      this.loading_post = true;
      this.forms.patchValue({
        tglpanjar: this.forms.value.tglpanjar !== null ? new Date(this.forms.value.tglpanjar).toLocaleDateString('en-US') : null,
