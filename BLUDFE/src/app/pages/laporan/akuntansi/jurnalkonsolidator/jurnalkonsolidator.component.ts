...
@Component({
  selector: 'app-jurnalkonsolidator',
  templateUrl: './jurnalkonsolidator.component.html',
  styleUrls: ['./jurnalkonsolidator.component.scss']
})
export class JurnalkonsolidatorComponent implements OnInit , OnDestroy {
		...
		if(e.cetak){
			// {
				...
				this.parameterReport = {
					...
					FileName: 'JuKonsol.rpt',
					Params: {
            '@tglawal': (this.tglAwal !== null || this.tglAwal != '') ? new Date(this.tglAwal).toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '',
            '@tglakhir': (this.tglAkhir !== null || this.tglAkhir != '') ? new Date(this.tglAkhir).toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '',
						'@tanggal': e.TGL,
						'@hal':e.halaman
					}
				};
