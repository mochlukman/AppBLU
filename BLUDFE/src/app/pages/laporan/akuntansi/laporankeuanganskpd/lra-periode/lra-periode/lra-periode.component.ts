...
@Component({
  selector: 'app-lra-periode',
  templateUrl: './lra-periode.component.html',
  styleUrls: ['./lra-periode.component.scss']
})
export class LraPeriodeComponent implements OnInit , OnDestroy {
  ...
  getTahap(){
    this.service.getsBykode('321,341').subscribe(resp => { //multi tahap
    ...
    if(e.cetak){
      if (!this.unitSelected) {
        ...
        this.parameterReport = {
          ...
          FileName: 'LRAPeriode.rpt',
          Params: {
            '@level':this.kdlevelSelected,
            '@tahap': this.tahapSelected,
            '@idUnit': this.unitSelected.idunit,
            '@tanggal': (this.tglAwal !== null || this.tglAwal != '') ? new Date(this.tglAwal).toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '',
            //'@tanggal': e.TGL,
            //'@hal':e.halaman
          }
        };
