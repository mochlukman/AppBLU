...
@Component({
  selector: 'app-lraskpdprog',
  templateUrl: './lraskpdprog.component.html',
  styleUrls: ['./lraskpdprog.component.scss']
})
export class LraskpdprogComponent implements OnInit, OnDestroy {
		...
		if(e.cetak){
			if (!this.unitSelected) {
				...
				this.parameterReport = {
					...
					FileName: 'lraskpdprog.rpt',
					Params: {
            '@level': 6,
            '@tahap': 321,
						'@idunit': this.unitSelected.idunit,
            '@tgl': (this.tglAwal !== null || this.tglAwal != '') ? new Date(this.tglAwal).toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '',
						'@tanggal': e.TGL,
						'@hal':e.halaman
					}
				};
