...
import {LookSp2bComponent} from 'src/app/shared/lookups/look-sp2b/look-sp2b.component';

@Component({
  selector: 'app-report-sp2b',
  templateUrl: './report-sp2b.component.html',
  styleUrls: ['./report-sp2b.component.scss'],
  providers: [DatePipe]
})
export class ReportSp2bComponent implements OnInit, OnDestroy {
  title: string = '';
  loading_post: boolean;
  uiUnit: IDisplayGlobal;
  uiSp2b: IDisplayGlobal;
  indoKalender: any;
  ...
  @ViewChild(LookSp2bComponent, {static: true}) sp2b : LookSp2bComponent;
  forms: FormGroup;
  initialForm: any;
  constructor(
    ...
    this.uiSp2b = {kode: '', nama: '' };
    this.forms = this.fb.group({
      idunit: [0, [Validators.required, Validators.min(1)]],
      idsp2b: [0, [Validators.required, Validators.min(1)]],
      idbulan: 0,
      tgl: null
    });
  ...
  lookSp2b(){
    if(this.forms.controls['idunit'].valid){
      this.sp2b.title = 'Pilih SP2B';
      this.sp2b.gets({idunit: this.forms.value.idunit});
      this.sp2b.showThis = true;
    } else {
  ...
  callbackSp2b(e: any){
    if(e){
      this.uiSp2b = {kode: e.nosp2b, nama: this.datePipe.transform(e.tglsp2b, 'dd/MM/yyyy')};
    } else {
      this.uiSp2b = {kode: '', nama: '' };
    }
    this.forms.patchValue({
    ...
    if(e.cetak){
      ...
      this.parameterReport = {
        ...
        Params: {
          ...
          '@bln': this.forms.value.idbulan,
          '@noSP2B': this.forms.value.nosp2b,
          '@tanggal': (this.forms.value.tgl !== null || this.forms.value.tgl != '') ? new Date(this.forms.value.tgl).toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '',
          //'@tgl': e.TGL,
          //'@hal':e.halaman
        }
      };
