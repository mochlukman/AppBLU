...
@Component({
  selector: 'app-bkubend-pajak-perjenis',
  templateUrl: './bkubend-pajak-perjenis.component.html',
  styleUrls: ['./bkubend-pajak-perjenis.component.scss']
})
export class BkubendPajakPerjenisComponent implements OnInit , OnDestroy {
  loading_post: boolean;
  uiUnit: IDisplayGlobal;
  unitSelected: IDaftunit;
  uiBend: IDisplayGlobal;
  pajakSelected: IPajak;
  optionPajak: SelectItem[];
  bendSelected: Ibend;
  ...
  this.service.gets().subscribe(resp => {
    ...
    if(resp.length > 0){
      resp.forEach(e => {
        this.optionPajak.push({label: e.kdpajak, value : e.idpajak});
      });
    }
  });
}
    ...
    if(e.cetak){
			if (!this.unitSelected) {
				...
				this.parameterReport = {
					...
					FileName: 'bkubpgpajakjenis.rpt',
					Params: {
            '@idUnit': this.unitSelected.idunit,
            '@idBend': this.bendSelected.idbend,
            '@idPajak': this.pajakSelected,
