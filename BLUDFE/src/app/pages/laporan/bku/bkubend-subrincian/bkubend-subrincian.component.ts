...
export class BkubendSubrincianComponent implements OnInit , OnDestroy {
  ...
  @ViewChild(LookDaftrekByRkarComponent, {static: true}) Daftrek : LookDaftrekByRkarComponent;
  ...
  lookDaftrek(){
    if(this.unitSelected && this.kegSelected){
      ...
      this.Daftrek.gets(this.unitSelected.idunit, this.kegSelected.data_id);
      this.Daftrek.showThis = true;
    } else {
      this.notif.warning('Unit Organiasi & Kegiatan harus dipilih');
    }
  }
