...
@Component({
  selector: 'app-aruskas-penerimaan-rincian',
  templateUrl: './aruskas-penerimaan-rincian.component.html',
  styleUrls: ['./aruskas-penerimaan-rincian.component.scss']
})
export class AruskasPenerimaanRincianComponent implements OnInit, OnDestroy, OnChanges {
  ...
  delete(e: any){
    this.notif.confir({
    	...
    	accept: () => {
    		this.service.delete(this.dataselected.idjnslak.toString(), e.idsetdlak).subscribe(
    			(resp) => {
    				if (resp.ok) {
              this.notif.success('Data berhasil dihapus');
              this.listdata = this.listdata.filter(f => f.idsetdlak !== e.idsetdlak);
