...
@Component({
  selector: 'app-aruskas-pengeluaran-rincian',
  templateUrl: './aruskas-pengeluaran-rincian.component.html',
  styleUrls: ['./aruskas-pengeluaran-rincian.component.scss']
})
export class AruskasPengeluaranRincianComponent implements OnInit, OnDestroy, OnChanges {
  ...
  delete(e: any){
    this.notif.confir({
    	...
    	accept: () => {
    		this.service.delete(this.dataselected.idjnslak.toString(), e.idsetrlak).subscribe(
    			(resp) => {
    				if (resp.ok) {
              this.notif.success('Data berhasil dihapus');
              this.listdata = this.listdata.filter(f => f.idsetrlak !== e.idsetrlak);
