...
@Component({
  selector: 'app-asset-hutang-form',
  templateUrl: './asset-hutang-form.component.html',
  styleUrls: ['./asset-hutang-form.component.scss']
})
export class AssetHutangFormComponent implements OnInit {
  showThis: boolean;
  title: string;
  msg: Message[];
  loading: boolean;
  ...
  gets(){
    this.loading = true;
    this.listdata = [];
    this.daftrekService.getByStartKode('2.1.06.').subscribe(resp => {     
      if(resp.length > 0){
  ...
  pilih(){
    let postbody = {
      idrekaset: this.idrekaset,
      idrekhutang: []
    };
    if(this.dataselected.length > 0){
      this.dataselected.forEach(f => {
        postbody.idrekhutang.push(f.idrek);
      });
