...
@Component({
  selector: 'app-asset-tetap-korolari-rekening-form',
  templateUrl: './asset-tetap-korolari-rekening-form.component.html',
  styleUrls: ['./asset-tetap-korolari-rekening-form.component.scss']
})
export class AssetTetapKorolariRekeningFormComponent implements OnInit {
  showThis: boolean;
  title: string;
  msg: Message[];
  loading: boolean;
  ...
  gets(){
    this.loading = true;
    this.listdata = [];
    this.daftrekService.getByStartKode('5.2').subscribe(resp => {     
      if(resp.length > 0){
