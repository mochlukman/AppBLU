...
@Component({
  selector: 'app-asset-tetap-korolari',
  templateUrl: './asset-tetap-korolari.component.html',
  styleUrls: ['./asset-tetap-korolari.component.scss']
})
export class AssetTetapKorolariComponent implements OnInit, OnDestroy {
  loading: boolean;
  ...
  get(){
    this.loading = true;
    this.listdata = [];
    this.service.getByStartKode('1.3.').subscribe(resp => {
      if(resp.length > 0){
        this.listdata = [...resp];
      } else {
