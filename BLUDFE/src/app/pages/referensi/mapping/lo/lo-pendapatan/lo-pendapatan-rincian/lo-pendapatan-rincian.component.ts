...
@Component({
  selector: 'app-lo-pendapatan-rincian',
  templateUrl: './lo-pendapatan-rincian.component.html',
  styleUrls: ['./lo-pendapatan-rincian.component.scss']
})
export class LoPendapatanRincianComponent implements OnInit, OnDestroy, OnChanges {
  ...
  delete(e: any){
    this.notif.confir({
    	...
    	accept: () => {
    		this.service.delete(this.dataselected.idjnsakun.toString(), e.idsetdlralo).subscribe(
    			(resp) => {
    				if (resp.ok) {
              this.notif.success('Data berhasil dihapus');
              this.listdata = this.listdata.filter(f => f.idsetdlralo !== e.idsetdlralo);
