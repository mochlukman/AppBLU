...
@Component({
  selector: 'app-rekening-pendapatan-rincian-form',
  templateUrl: './rekening-pendapatan-rincian-form.component.html',
  styleUrls: ['./rekening-pendapatan-rincian-form.component.scss']
})
export class RekeningPendapatanRincianFormComponent implements OnInit {
  showThis: boolean;
  title: string;
  msg: Message[];
  loading: boolean;
  ...
  gets(){
    this.loading = true;
    this.listdata = [];
    this.daftrekService.get({Idjnsakun: 5}).subscribe(resp => {
      if(resp.length > 0){
