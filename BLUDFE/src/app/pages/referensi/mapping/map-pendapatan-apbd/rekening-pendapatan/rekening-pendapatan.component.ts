...
@Component({
  selector: 'app-rekening-pendapatan',
  templateUrl: './rekening-pendapatan.component.html',
  styleUrls: ['./rekening-pendapatan.component.scss']
})
export class RekeningPendapatanComponent implements OnInit, OnDestroy {
  loading: boolean;
  ...
  get(){
    this.loading = true;
    this.listdata = [];
    this.service.get({Idjnsakun: 4}).subscribe(resp => {
      if(resp.length > 0){
        this.listdata = [...resp];
      } else {
