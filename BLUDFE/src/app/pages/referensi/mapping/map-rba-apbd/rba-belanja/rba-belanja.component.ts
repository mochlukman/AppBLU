...
@Component({
  selector: 'app-rba-belanja',
  templateUrl: './rba-belanja.component.html',
  styleUrls: ['./rba-belanja.component.scss']
})
export class RbaBelanjaComponent implements OnInit, OnDestroy {
  loading: boolean;
  ...
  get(){
    this.loading = true;
    this.listdata = [];
    this.service.get({Kdkhusus: 2, Idjnsakun: 5}).subscribe(resp => {
      if(resp.length > 0){
        this.listdata = [...resp];
      } else {
