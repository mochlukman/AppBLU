...
@Component({
  selector: 'app-rba-pendapatan-rincian-form',
  templateUrl: './rba-pendapatan-rincian-form.component.html',
  styleUrls: ['./rba-pendapatan-rincian-form.component.scss']
})
export class RbaPendapatanRincianFormComponent implements OnInit {
  showThis: boolean;
  title: string;
  msg: Message[];
  loading: boolean;
  ...
  gets(){
    this.loading = true;
    this.listdata = [];
    this.daftrekService.get({Kdkhusus: 1, Idjnsakun: 4}).subscribe(resp => {
      if(resp.length > 0){
