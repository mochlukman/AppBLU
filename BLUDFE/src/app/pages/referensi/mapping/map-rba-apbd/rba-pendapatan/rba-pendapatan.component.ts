...
@Component({
  selector: 'app-rba-pendapatan',
  templateUrl: './rba-pendapatan.component.html',
  styleUrls: ['./rba-pendapatan.component.scss']
})
export class RbaPendapatanComponent implements OnInit, OnDestroy {
  loading: boolean;
  ...
  get(){
    this.loading = true;
    this.listdata = [];
    this.service.get({Kdkhusus: 2, Idjnsakun: 4}).subscribe(resp => {
      if(resp.length > 0){
        this.listdata = [...resp];
      } else {
