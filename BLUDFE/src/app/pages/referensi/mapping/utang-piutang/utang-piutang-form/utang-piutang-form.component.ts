...
@Component({
  selector: 'app-utang-piutang-form',
  templateUrl: './utang-piutang-form.component.html',
  styleUrls: ['./utang-piutang-form.component.scss']
})
export class UtangPiutangFormComponent implements OnInit {
  showThis: boolean;
  title: string;
  msg: Message[];
  loading: boolean;
  ...
  pilih(){
    ...
    if(this.dataselected.length > 0){
      this.dataselected.forEach(f => {
        postbody.idreknrc.push(f.idrek);
      });
