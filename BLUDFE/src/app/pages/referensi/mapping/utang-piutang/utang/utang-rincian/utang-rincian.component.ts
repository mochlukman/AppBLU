...
@Component({
  selector: 'app-utang-rincian',
  templateUrl: './utang-rincian.component.html',
  styleUrls: ['./utang-rincian.component.scss']
})
export class UtangRincianComponent implements OnInit, OnChanges, OnDestroy {
  ...
  gets(){
    if(this.dataselected){
      this.loading = true;
      this.listdata = [];
      this.service.gets('8', this.dataselected.idrek).subscribe(resp => {
        if(resp.length > 0){
          this.listdata = resp;
        } else {
  ...
  delete(e: any){
    this.notif.confir({
    	...
    	accept: () => {
    		this.service.delete('8', e.idsetuprlo).subscribe(
    			(resp) => {
    				if (resp.ok) {
              this.notif.success('Data berhasil dihapus');
              this.listdata = this.listdata.filter(f => f.idsetuprlo !== e.idsetuprlo);
