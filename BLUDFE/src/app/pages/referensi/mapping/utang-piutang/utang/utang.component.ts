...
@Component({
  selector: 'app-utang',
  templateUrl: './utang.component.html',
  styleUrls: ['./utang.component.scss']
})
export class UtangComponent implements OnInit, OnDestroy {
  loading: boolean;
  ...
  get(){
    this.loading = true;
    this.listdata = [];
    this.service._idjnsakun = '1';
    this.service._nmper = 'Utang';
    this.service.getByJnsakunAndNmper().subscribe(resp => {
      if(resp.length > 0){
        this.listdata = [...resp];
      } else {
